...
class Connection(object):
    ...
    def _check_reply(self, reply):
        if reply.reply_type != _C.AMQP_RESPONSE_NORMAL:
            raise Exception(
                "reply_type=%d, library_error=%d"
                % (reply.reply_type, reply.library_error)
            )
    ...
    def declare_queue(
        ...
        ok = _C.amqp_queue_declare(
            self._state,
            channel,
            _str_to_amqp_bytes(queue),
            passive,
            durable,
            exclusive,
            auto_delete,
    ...
    def bind_queue(self, channel, queue, exchange, routing_key=None):
        ok = _C.amqp_queue_bind(
            self._state,
            channel,
            _str_to_amqp_bytes(queue),
            _str_to_amqp_bytes(exchange),
            _str_to_amqp_bytes(routing_key),
            _C.amqp_empty_table,
        )
        reply = _C.amqp_get_rpc_reply(self._state)
        self._check_reply(reply)
    ...
    def declare_exchange(self, channel, exchange, type, passive=False, durable=False):
        ok = _C.amqp_exchange_declare(
            self._state,
            channel,
            _str_to_amqp_bytes(exchange),
            _str_to_amqp_bytes(type),
            passive,
            durable,
            _C.amqp_empty_table,
        )
    ...
    def consume(
        ...
        ok = _C.amqp_basic_consume(
            self._state,
            channel,
            _str_to_amqp_bytes(queue),
            _str_to_amqp_bytes(consumer_tag),
            no_local,
            no_ack,
            exclusive,
            _C.amqp_empty_table,
...
class Envelope(object):
    ...
    def __str__(self):
        return "<Envelope: channel={0.channel}, consumer_tag={0.consumer_tag}, delivery_tag={0.delivery_tag}, redelivered={0.redelivered}, exchange={0.exchange}, routing_key={0.routing_key}, message={0.message}>".format(
            self
        )
...
class Properties(object):
    ...
    def from_amqp_properties(cls, props):
        return cls(
            content_type=_amqp_bytes(props.content_type).decode("utf-8"),
    ...
    def __str__(self):
        return "<Properties: flags={0.flags}, content_type={0.content_type}, content_encoding={0.content_encoding}>".format(
            self
        )
...
class Message(object):
    ...
    def __str__(self):
        return (
            "<Message: properties={0.properties}, body={0.body}, pool={0.pool}>".format(
                self
            )
        )
